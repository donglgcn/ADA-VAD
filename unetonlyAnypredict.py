...
from evaluation_anypredict import evaluate
...
def train(config, training_chunked_samples_dir, testing_chunked_samples_file):
    ...
    model = SelfCompleteNet1raw1ofAnyPredict(**config["model_paras"]).to(device)
    ...
    for epoch in range(epoch_last, epochs + epoch_last):
        for chunk_file_idx, chunk_file in enumerate(training_chunk_samples_files):
            ...
            for idx, train_data in tqdm(enumerate(dataloader),
                ...
                of_output, raw_output, of_target, raw_target,_,_ = model(sample_frames, sample_ofs)
                ...
                if step % config["logevery"] == config["logevery"] - 1:
                    ...
                    writer.add_scalar('loss_total/train', loss_recon, global_step=step + 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-f", "--file", dest="config_file", help="config yaml file address",
                        default="./cfgs/unetOnly_triway.yaml")

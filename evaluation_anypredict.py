...
import numpy as np
import yaml
from models.unet import SelfCompleteNet1raw1of, SelfCompleteNet1raw1ofAnyPredict
...
def evaluate(config, ckpt_path, testing_chunked_samples_file, training_stats_path, suffix, dataset_name=None):
    ...
    model = SelfCompleteNet1raw1ofAnyPredict(**config["model_paras"]).to(device).eval()
    ...
    for test_data in tqdm(dataloader_test, desc="Eval: ", total=len(dataloader_test)):
        ...
        of_output, raw_output, of_target, raw_target,_,_ = model(sample_frames_test, sample_ofs_test)
        # todo
    ...
    os.makedirs(os.path.join(config["eval_root"], config["exp_name"], dataset_name), exist_ok=True)
    joblib.dump(frame_scores,
                os.path.join(config["eval_root"], config["exp_name"], dataset_name, "frame_scores_%s.json" % suffix))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_save_path", type=str,
                        default="/data/dongliang/savedmodels/interpretableVAD/ckpt/shtech_unetOnly_P3_2/best.pth",
                        help='path to pretrained weights')
    parser.add_argument("--cfg_file", type=str,
                        default="./cfgs/unetonlyAnypredict/unetOnly_shtechP3_2.yaml",
                        help='path to pretrained model configs')
    args = parser.parse_args()

    config = yaml.safe_load(open(args.cfg_file))
    train_dataset_root = config["dataset_base_dir"]   # "/home/dongliang/datasets/VAD"
    test_dataset_root = "/data/dongliang/datasets/VAD/UCSDped"
    test_dataset_name = "ped2"
    testing_chunked_samples_file = os.path.join(test_dataset_root, test_dataset_name,
                                                "testing/chunked_samples/chunked_samples_00.pkl")

    from unetonlyAnypredict import cal_training_stats

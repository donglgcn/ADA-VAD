...
if __name__ == '__main__':
    ...
    parser.add_argument("--proj_root", type=str, default="/home/dongliang/PHD/research/code/hf2vad-master", help='project root path')
    ...
    dataset_root = "/home/dongliang/datasets/VAD/UCSDped"

    all_bboxes = np.load(
        # os.path.join(args.proj_root, "data", args.dataset_name, '%s_bboxes_%s.npy' % (args.dataset_name, args.mode)),
        os.path.join(dataset_root, args.dataset_name, '%s_bboxes_%s.npy' % (args.dataset_name, args.mode)),
        allow_pickle=True
    )
    if args.mode == "train":
        # save_dir = os.path.join(args.proj_root, "data", args.dataset_name, "training", "chunked_samples")
        save_dir = os.path.join(dataset_root, args.dataset_name, "training", "chunked_samples")
    else:
        # save_dir = os.path.join(args.proj_root, "data", args.dataset_name, "testing", "chunked_samples")
        save_dir = os.path.join(dataset_root, args.dataset_name, "testing", "chunked_samples")

    samples_extraction(
        dataset_root=dataset_root,#os.path.join(args.proj_root, "data"),
